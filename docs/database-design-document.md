...
export const workflowStatuses = ['created', 'running', 'paused', 'editing', 'completed', 'failed', 'cancelled'] as const;
...
export const workflowRepositories = sqliteTable('workflow_repositories', {
  id: integer('id').primaryKey({ autoIncrement: true }),
...
export const stepTypes = [
  ...
  'routing', 'implementation', 'quality_gate', 'gemini_review'
] as const;
...
export const workflowSteps = sqliteTable('workflow_steps', {
  ...
  index('workflow_steps_workflow_step_number_idx').on(table.workflowId, table.stepNumber),
]);
```
...
export const discoveredFiles = sqliteTable('discovered_files', {
  id: integer('id').primaryKey({ autoIncrement: true }),
...
export const agentTools = sqliteTable('agent_tools', {
  ...
  uniqueIndex('agent_tools_unique_idx').on(table.agentId, table.toolName, table.toolPattern),
]);
```
...
export const templateCategories = ['data', 'ui', 'backend', 'security', 'electron'] as const;
...
export const templatePlaceholders = sqliteTable('template_placeholders', {
  ...
  uniqueIndex('template_placeholders_unique_idx').on(table.templateId, table.name),
]);
```
...
export const implementationPlanSteps = sqliteTable('implementation_plan_steps', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  ...
  agentId: integer('agent_id')
    .references(() => agents.id, { onDelete: 'set null' }),
  agentOverrideId: integer('agent_override_id')
    .references(() => agents.id, { onDelete: 'set null' }),
  stepNumber: integer('step_number').notNull(),
  title: text('title').notNull(),
  description: text('description'),
...
export const implementationPlanStepFiles = sqliteTable('implementation_plan_step_files', {
  id: integer('id').primaryKey({ autoIncrement: true }),
...
export const settingCategories = ['workflow', 'worktree', 'logging', 'ui', 'advanced'] as const;
