...
interface ContentSectionProps extends Omit<
    ...
    <div className={cn("flex flex-col gap-4", className)} ref={ref} {...props}>
      ...
      {hasActions && (
        <div className={"flex items-center gap-2"}>
          ...
          {isCanRetry && (
            <Button
              disabled={regenerateStepMutation.isPending}
              onClick={handleRetry}
              size={"sm"}
              variant={"outline"}
            >
              <RefreshCw
                ...
                className={cn(
                  "mr-1.5 size-3.5",
                  regenerateStepMutation.isPending && "animate-spin"
                )}
              />
        ...
        {isOutputEdited && (
          <div className={"flex items-center gap-2"}>
            ...
            <span className={"text-xs text-muted-foreground"}>
              Modified on {formatTimestamp(outputEditedAt)}
            </span>
          </div>
        )}
