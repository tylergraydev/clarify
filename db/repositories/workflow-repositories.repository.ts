...
export function createWorkflowRepositoriesRepository(
  db: DrizzleDatabase
  ...
  return {
    ...
    removeFromWorkflow(workflowId: number, repositoryId: number): boolean {
      const result = db
        ...
        .where(
          and(
            eq(workflowRepositories.workflowId, workflowId),
            eq(workflowRepositories.repositoryId, repositoryId)
          )
        )
        .run();
