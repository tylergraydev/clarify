...
import type { Workflow, WorkflowHistorySortField, WorkflowHistorySortOrder } from '@/types/electron';
...
export const WorkflowHistoryTable = ({
  ...
  const renderSortableHeader = (column: SortableColumn) => {
    ...
    return (
      <th className={'px-4 py-3 text-left font-medium text-muted-foreground'} key={column.field} scope={'col'}>
        <button
      ...
      <table className={'w-full border-collapse text-sm'}>
        ...
        <tbody className={'divide-y divide-border'}>
          {workflows.map((workflow) => {
            ...
            return (
              <tr
                className={cn('cursor-pointer transition-colors hover:bg-muted/30', isFailed && 'opacity-75')}
                key={workflow.id}
              ...
              >
                ...
                {/* Duration Cell */}
                <td className={'px-4 py-3 whitespace-nowrap text-muted-foreground'}>{formattedDuration}</td>

                {/* Completed Date Cell */}
                <td className={'px-4 py-3 whitespace-nowrap text-muted-foreground'}>{formattedCompletedDate}</td>
