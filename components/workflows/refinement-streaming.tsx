...
export const refinementStreamingVariants = cva(
  `
 ...
 */
type ExtendedRefinementOutcome = RefinementOutcome & RefinementOutcomePauseInfo;

/**
 * Extended outcome fields for pause and retry information.
 */
interface RefinementOutcomePauseInfo {
  isPaused?: boolean;
  retryCount?: number;
  skipFallbackAvailable?: boolean;
}

interface RefinementStreamingProps
  extends Omit<ComponentProps<'div'>, 'onError'>, VariantProps<typeof refinementStreamingVariants> {
  ...
  onCancel?: () => void;
  /** Callback when a refinement error occurs */
  onRefinementError?: (error: string) => void;
  /** Callback when retry button is clicked */
  onRetry?: () => void;
  /** Callback when skip refinement button is clicked */
  onSkipRefinement?: (reason: string) => void;
  /** Callback when refinement succeeds */
  onSuccess?: (refinedText: string) => void;
  /** Outcome of the refinement process */
  outcome?: ExtendedRefinementOutcome | null;
...
/**
 * RefinementStreaming displays live agent output during the refinement step.
 * Shows streaming text, tool use indicators, thinking blocks, and progress phases.
 */
export const RefinementStreaming = memo(
  ({
    activeTools = [],
    ...
    toolHistory = [],
    ...props
  }: RefinementStreamingProps): ReactElement => {
    ...
    const canSkip = onSkipRefinement && outcome?.skipFallbackAvailable !== false;
    ...
    useEffect(() => {
      ...
      if (outcome.type === 'SUCCESS' && onSuccess) {
        onSuccess(outcome.refinedText);
      } else if ((outcome.type === 'ERROR' || outcome.type === 'TIMEOUT') && onRefinementError) {
        onRefinementError(outcome.error);
      }
    }, [outcome, onSuccess, onRefinementError]);
    ...
    const handleSkipClick = useCallback(() => {
      ...
      onSkipRefinement?.(`Skipped due to error: ${errorMessage}`);
    }, [onSkipRefinement, error, outcome]);
    ...
    return (
      <div className={cn(refinementStreamingVariants({ layout, status: resolvedStatus }), className)} {...props}>
        {/* Header Section */}
              ...
              <span className={'text-sm font-medium'}>
                {isRunning ? (
                  <Shimmer duration={isExtendedThinking ? 2.5 : 1.5}>
                    {agentName} is {isExtendedThinking ? 'deeply reasoning' : 'refining'}...
                  </Shimmer>
                ) : (
                  <Fragment>
                    {headerTitle}
            ...
            <StickToBottom.Content className={'flex flex-col p-4'}>
              ...
              {isRunning && !text && (
                <div className={'flex items-center gap-2 text-sm text-muted-foreground'}>
                  ...
                  <span>Refining feature request based on clarification answers...</span>
                </div>
              )}
              ...
              {/* Empty State */}
              {isEmptyState && <div className={'text-sm text-muted-foreground'}>Ready to start refinement...</div>}
...
RefinementStreaming.displayName = 'RefinementStreaming';
...
const ToolIndicator = memo(({ tool, variant = 'active' }: ToolIndicatorProps): ReactElement => {
  ...
  const hasNoFormattedContent = !summary && details.length === 0;
  const shouldShowRawPreview = hasNoFormattedContent && rawPreview;
  const shouldShowPreparingMessage = hasNoFormattedContent && !rawPreview;
  ...
  return (
    ...
    >
      ...
      {shouldShowPreparingMessage && !isHistory && (
