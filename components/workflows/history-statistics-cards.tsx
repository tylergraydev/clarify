...
const StatisticCard = ({
  ...
  return (
    <div className={cn('flex items-center gap-4 rounded-lg border border-card-border bg-card p-4', className)}>
      {/* Icon */}
      <div className={cn('flex size-10 shrink-0 items-center justify-center rounded-full', iconColorClass)}>{icon}</div>
...
export const HistoryStatisticsCards = ({ className, isLoading = false, statistics }: HistoryStatisticsCardsProps) => {
  ...
  return (
    <div
      aria-busy={isLoading}
      aria-label={isLoading ? 'Loading statistics' : 'Workflow history statistics'}
      aria-live={'polite'}
