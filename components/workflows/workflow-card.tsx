...
interface WorkflowCardProps extends Omit<ComponentPropsWithRef<'div'>, 'onClick'> {
...
export const WorkflowCard = ({
  ...
  const progressPercentage =
    workflow.totalSteps && workflow.totalSteps > 0
      ? Math.round(((workflow.currentStepNumber ?? 0) / workflow.totalSteps) * 100)
      : 0;
  ...
  return (
    <Card className={cn('flex flex-col transition-opacity', className)} ref={ref} {...props}>
      {/* Header */}
      <CardHeader>
        <div className={'flex items-start justify-between gap-2'}>
          <CardTitle className={'line-clamp-1'}>{workflow.featureName}</CardTitle>
          <Badge variant={getStatusVariant(workflow.status as WorkflowStatus)}>
            {formatStatusLabel(workflow.status as WorkflowStatus)}
          </Badge>
        </div>
        {projectName && <CardDescription className={'line-clamp-1'}>{projectName}</CardDescription>}
      </CardHeader>
      ...
      <CardFooter className={'gap-2'}>
        <Button aria-label={'View workflow details'} onClick={handleViewDetailsClick} size={'sm'} variant={'outline'}>
          <Eye aria-hidden={'true'} className={'size-4'} />
          {'View'}
        </Button>
        {isCancellable && (
          <Button aria-label={'Cancel workflow'} onClick={handleCancelClick} size={'sm'} variant={'ghost'}>
