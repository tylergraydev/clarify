 ...
 */
const getStatusVariant = (status: string): 'completed' | 'default' | 'failed' => {
  switch (status) {
    case 'cancelled':
    case 'failed':
      return 'failed';
...
const WorkflowItemSkeleton = () => {
  return (
    <div
      ...
      className={'animate-pulse border-b border-card-border p-3 last:border-b-0'}
      role={'article'}
    >
...
const RecentWorkflowsContent = () => {
  ...
  const recentWorkflows = useMemo(() => {
    return workflows
      .filter(
        (workflow) => workflow.status === 'completed' || workflow.status === 'failed' || workflow.status === 'cancelled'
      )
      .sort((a, b) => {
  ...
  return (
    <Card className={className}>
      <CardHeader>
        ...
        <CardDescription>Recently completed, failed, or cancelled workflows</CardDescription>
      </CardHeader>
      <CardContent>
        <QueryErrorBoundary>
